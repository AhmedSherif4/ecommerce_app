...
abstract class BaseRemoteDataSource {
  ...
  Future<Map<String, dynamic>> postDataBackParent({
    required String url,
    required Object body,
  });
  ...
  Future<Map<String, dynamic>> get({required String url, Options? options});
}
...
class BaseRemoteDataSourceImpl implements BaseRemoteDataSource {
  ...
  Future<Map<String, dynamic>> postData({
    ...
    final response = await _checkStatusCode(
      await apiConsumer.post(url, body: body, options: options),
    );
    //response.log();
    return response;
  }
  ...
  Future<Map<String, dynamic>> postDataBackParent({
    ...
    final response = await _checkStatusCode(
      await apiConsumer.postDataBackParent(url, body: body),
    );
    return response;
  }
  ...
  Future<Map<String, dynamic>> publicPostData({
    ...
    final response = await _checkStatusCode(
      await apiConsumer.publicPost(url, body: body),
    );
    return response;
  }
  ...
  Future<Map<String, dynamic>> get({
    ...
    return await _checkStatusCode(await apiConsumer.get(url, options: options));
  }
  ...
  Future<Map<String, dynamic>> postDataForPayment({
    ...
    final response = await _checkPaymentStatusCode(
  ...
  Future<Map<String, dynamic>> _checkPaymentStatusCode(
    ...
    if (response.statusCode != 200) {
      throw ServerException(
        message:
            jsonDecode(response.data)['result']['description'] ??
            AppStrings.someThingWentWrong,
      );
    }
    final json = jsonDecode(response.data);
