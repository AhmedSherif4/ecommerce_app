...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  LoginBloc({required this.loginUseCase, required this.socialLoginUseCase})
        ...
        (data) async {
          emit(
            state.copyWith(
              ...
              loginMessage: AppStrings.loginSuccessfully,
            ),
          );
          userData = data;
        },
    ...
    on<SocialLoginEvent>((SocialLoginEvent event, emit) async {
      emit(
        state.copyWith(
      ...
      result.fold(
        (failure) {
          emit(state.copyWith(socialLoginStates: RequestStates.error));
        },
        (data) {
          emit(state.copyWith(socialLoginStates: RequestStates.loaded));
          userData = data;
        },
      );
    });
