...
class ImagePickBloc extends Bloc<ImagePickEvent, ImagePickState> {
  ...
  void _getImage(ImagePickerEvent event, Emitter<ImagePickState> emit) async {
    emit(state.copyWith(pickImageState: PickImageState.startLoadingImage));
    ...
    if (pickedFile == null) {
      if (imageFile != null) {
        ...
        emit(
          state.copyWith(
            pickImageState: PickImageState.imagePickedError,
            pickImageErrorMessage: AppStrings.noImageSelected,
          ),
        );
        return null;
      }
      ...
      if (!isPngOrJpg) {
        emit(
          state.copyWith(
            ...
            pickImageErrorMessage: AppStrings.invalidImageFormat,
          ),
        );
        return;
      }
  ...
  Future<File> _cropImage(File file) async {
    final croppedFile = await ImageCropper().cropImage(
      ...
      uiSettings: [
        AndroidUiSettings(
          toolbarTitle: '',
          // toolbarColor: AppColors.primaryColorBlue,
  ...
  Future<File> _resizeImage(File file, int maxSizeInMB) async {
    ...
    final resizedFile = File(
      file.path.replaceAll(RegExp(r'\.\w+$'), '_resized.jpg'),
    );
  ...
  FutureOr<void> _resetImagePicker(
    ResetImagePickerEvent event,
    Emitter<ImagePickState> emit,
  ) {
    imageFile = null;
    _croppedFile = null;
    emit(
